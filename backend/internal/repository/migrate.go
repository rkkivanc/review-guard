...
// Migrate applies embedded SQL migrations in lexical order inside a transaction.
func Migrate(ctx context.Context, pool *pgxpool.Pool) error {
	...
	for _, name := range names {
		...
		sql := string(raw)
		// Strip goose-style headers if present.
		sql = stripMigrationHeader(sql)
...
func stripMigrationHeader(sql string) string {
